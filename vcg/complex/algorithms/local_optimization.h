...
namespace vcg {
	...
	enum ModifierType {
		TetraEdgeCollapseOp, TriEdgeSwapOp, TriVertexSplitOp,
	...
	public:
		...
		template <class LocalModificationType, class LocalTexHelperType, class LocalVertexPointerType>
		void Init(int initMark, LocalTexHelperType& qTH, std::shared_ptr<std::vector<LocalVertexPointerType>> wv)
		{
		    vcg::tri::InitVertexIMark(m);
			...
			LocalModificationType::Init(m, h, pp, initMark, qTH, wv);
		    std::make_heap(h.begin(), h.end());
		    if (!h.empty()) currMetric = h.front().pri;
		}
		...
		template <class LocalModificationType, class LocalVertexPointerType>
		void Finalize(std::shared_ptr<std::vector<LocalVertexPointerType>> wv)
		{
			LocalModificationType::Finalize(m, h, pp, wv);
		}
