...
#include <vcg/complex/algorithms/local_optimization/info_standard.h>
...
namespace vcg
{
	namespace tri
	{
		...
		public:
			...
			void Alloc(VertexType* v, vcg::TexCoord2f& coord)
			{
			...
			void SumAll(VertexType* v, vcg::TexCoord2f& coord, Quadric5<double>& q)
			{
				std::vector<std::pair<vcg::TexCoord2f, Quadric5<double> > >& qv = Vd(v);
			...
			bool Contains(VertexType* v, vcg::TexCoord2f& coord)
			{
				std::vector<std::pair<vcg::TexCoord2f, Quadric5<double> > >& qv = Vd(v);
			...
			Quadric5<double>& Qd(VertexType* v, const vcg::TexCoord2f& coord)
			{
				std::vector<std::pair<vcg::TexCoord2f, Quadric5<double> > >& qv = Vd(v);
				//assert(coord.N()>=0);
				for (size_t i = 0; i < qv.size(); i++)
		...
		public:
			math::Quadric<double>& Qd3(VertexType* v) {
				return TD3()[*v];
			}
			math::Quadric<double>& Qd3(VertexType& v) {
				return TD3()[v];
			...
			std::vector<std::pair<vcg::TexCoord2f, Quadric5<double> > >& Vd(VertexType* v) {
				return (TD()[*v]);
			}
			typename VertexType::ScalarType W(VertexType* /*v*/) {
				return 1.0;
			}
			typename VertexType::ScalarType W(VertexType& /*v*/) {
				return 1.0;
			}
			void Merge(VertexType& /*v_dest*/, VertexType const& /*v_del*/) {}
			Quadric5Temp*& TDp() {
		...
		class TriEdgeCollapseQuadricTex : public vcg::tri::TriEdgeCollapseThreadSafe< TriMeshType, VertexPair, MYTYPE>
		{
			typedef HelperType QH;
			typedef typename tri::TriEdgeCollapseThreadSafe<TriMeshType, VertexPair, MYTYPE>::HeapType HeapType;
			typedef typename tri::TriEdgeCollapseThreadSafe<TriMeshType, VertexPair, MYTYPE>::HeapElem HeapElem;
		...
		private:
			std::shared_ptr<std::vector<VertexPointer>> _WV;
			TriEdgeCollapseQuadricTexParameter p;

			HelperType& _qH;

		public:

			inline TriEdgeCollapseQuadricTex(const VertexPair& p,
				std::shared_ptr<int> mark, BaseParameterClass* _pp,
				HelperType& qh,
				std::shared_ptr<std::vector<VertexPointer>> wv)
				: _qH(qh)
				, _WV(wv)
			{
				TriEdgeCollapseQuadricTexParameter* pp = (TriEdgeCollapseQuadricTexParameter*)_pp;
				this->localMark = *mark;
				this->MarkUpDating() = mark;
				this->pos = p;
				this->_priority = ComputePriority(pp);
			}

			// puntatori ai vertici che sono stati messi non-w per preservare il boundary
			std::vector<VertexPointer>& WV() {
				//static std::vector<VertexPointer> _WV;
				return _WV;
			};
			...
			static void Finalize(TriMeshType& m, HeapType& /*h_ret*/, BaseParameterClass* _pp, std::shared_ptr<std::vector<VertexPointer>> wv)
			{
				TriEdgeCollapseQuadricTexParameter* pp = (TriEdgeCollapseQuadricTexParameter*)_pp;
				...
				if (pp->PreserveBoundary)
				{
					...
					for (wvi = /*WV().*/wv->begin(); wvi != /*WV().*/wv->end(); ++wvi)
						if (!(*wvi)->IsD())
							(*wvi)->SetW();
				}
			}
			...
			*/
			ScalarType ComputeTexPriority(const double vv[5], Quadric5<double>& qsum, BaseParameterClass* _pp)
			{
				...
				if (QuadErr < 1e-15)
					QuadErr = 1e-15; // Do not use zero quality penalties
				...
				BaseParameterClass* _pp)
			{
				...
				assert(_qH.Qd(this->pos.V(0), tcoord0_1).IsValid());
				assert(_qH.Qd(this->pos.V(1), tcoord1_1).IsValid());

				qsum_1 = _qH.Qd(this->pos.V(0), tcoord0_1);
				qsum_1 += _qH.Qd(this->pos.V(1), tcoord1_1);

				ComputeMinimal(dest_1, tmp1, tmp2, qsum_1, pp);
				priority1 = ComputeTexPriority(dest_1, qsum_1, pp);

				if (ncoords < 2)
					return priority1 * (1 + (pp->ExtraTCoordWeight) * (_qH.Vd(this->pos.V(0)).size() + _qH.Vd(this->pos.V(1)).size() - 2));
				...
				assert(_qH.Qd(this->pos.V(0), tcoord0_2).IsValid());
				assert(_qH.Qd(this->pos.V(1), tcoord1_2).IsValid());

				qsum_2 = _qH.Qd(this->pos.V(0), tcoord0_2);
				qsum_2 += _qH.Qd(this->pos.V(1), tcoord1_2);
				...
				this->_priority = std::max(priority1, priority2) * (1 + (pp->ExtraTCoordWeight) * (_qH.Vd(this->pos.V(0)).size() + _qH.Vd(this->pos.V(1)).size() - 2));
			...
			static void InitQuadric(TriMeshType& m, BaseParameterClass* _pp, HelperType& qh)
			{
				tri::TriEdgeCollapseQuadricTexParameter* pp = (tri::TriEdgeCollapseQuadricTexParameter*)_pp;
					...
					if (!(*pf).IsD() && (*pf).IsR())
					{
						if ((*pf).V(0)->IsR() && (*pf).V(1)->IsR() && (*pf).V(2)->IsR())
						{
							...
							for (int j = 0; j < 3; ++j)
							{
								if ((*pf).V(j)->IsW())
								{
									if (!qh.Contains((*pf).V(j), (*pf).WT(j)))
									{
										qh.Alloc((*pf).V(j), (*pf).WT(j));
									}
									assert(!math::IsNAN((*pf).WT(j).u()));
									assert(!math::IsNAN((*pf).WT(j).v()));
									qh.SumAll((*pf).V(j), (*pf).WT(j), q);
								}
							}
						}
					}
			...
			static void Init(TriMeshType& m,
				HeapType& h_ret,
				BaseParameterClass* _pp,
				int globMark,
				HelperType& qh,
				std::shared_ptr<std::vector<VertexPointer>> wv)
			{
						...
						if (!(*pf).IsD() && (*pf).IsW())
						{
							for (int j = 0; j < 3; ++j)
							{
								if ((*pf).IsB(j))
								{
									...
									{
										(*pf).V(j)->ClearW();
										/*WV().*/wv->push_back((*pf).V(j));
									}
									if ((*pf).V1(j)->IsW())
									{
										(*pf).V1(j)->ClearW();
										/*WV().*/wv->push_back((*pf).V1(j));
									}
								}
							}
						}
				...
				std::shared_ptr<int> globalMark = std::make_shared<int>(globMark);
					...
					if ((*vi).IsRW())
					{
						...
						for (x.F() = (*vi).VFp(), x.I() = (*vi).VFi(); x.F() != 0; ++x)
						{
							...
							if ((x.V0() < x.V1()) && x.V1()->IsRW() && !x.V1()->IsV()) {
								...
								h_ret.push_back(HeapElem(new MYTYPE(
									VertexPair(x.V0(), x.V1()),
									globalMark/*TriEdgeCollapse< TriMeshType, VertexPair, MYTYPE>::GlobalMark()*/,
									pp,
									qh,
									wv)
								));
			...
			void Execute(TriMeshType& m, BaseParameterClass* _pp)
			{
				...
				math::Quadric<double> qsum3 = _qH.Qd3(v[0]);
				qsum3 += _qH.Qd3(v[1]);
				...
				while (!vfi.End())
				{
					...
					else
					{
						...
						if (_qH.Contains(v[0], tcoords))
						{
							newq = _qH.Qd(v[0], tcoords);
							newq.Sum3(_qH.Qd3(v[1]), tcoords.u(), tcoords.v());
						}
						else if (_qH.Contains(v[1], tcoords))
						{
							newq = _qH.Qd(v[1], tcoords);
							newq.Sum3(_qH.Qd3(v[0]), tcoords.u(), tcoords.v());
						}
						else
							assert(0);
