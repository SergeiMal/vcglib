...
namespace vcg {
	namespace tri {
		...
		protected:
			...
			std::shared_ptr<int>& MarkUpDating() {
				return mGlobalMark;
			}
		...
		private:
			std::shared_ptr<int> mGlobalMark;
		...
		template<class TriMeshType, class VertexPair, class MYTYPE>
		class TriEdgeCollapseThreadSafe : public LocalOptimization<TriMeshType>::LocModType
		{
		public:
			/// static data to gather statistical information about the reasons of collapse failures
			class FailStat {
			private:
				...
				int bor = 0;
			public:
				/*static*/ int& Volume() { return vol; }
				/*static*/ int& LinkConditionFace() { return lkf; }
				/*static*/ int& LinkConditionEdge() { return lke; }
				/*static*/ int& LinkConditionVert() { return lkv; }
				/*static*/ int& OutOfDate() { return ofd; }
				/*static*/ int& Border() { return bor; }
				/*static*/ void Init()
				{
					Volume() = 0;
					LinkConditionFace() = 0;
		...
		public:
			/// Default Constructor
			inline	TriEdgeCollapseThreadSafe()
			{}
			///Constructor with postype
			inline TriEdgeCollapseThreadSafe(const VertexPair& p, int mark, BaseParameterClass* pp)
			{
				localMark = mark;
				pos = p;
				_priority = ComputePriority(pp);
		...
		public:
			...
			template <class LocalTexHelperType>
			inline  void UpdateHeap(HeapType& h_ret, BaseParameterClass* pp, LocalTexHelperType& qh)
			{
				(*MarkUpDating())++;
				VertexType* v[2];
				v[0] = pos.V(0); v[1] = pos.V(1);
				v[1]->IMark() = *(MarkUpDating());
				...
				while (!vfi.End())
				{
					...
					{
						vfi.V1()->SetV();
						h_ret.push_back(HeapElem(new MYTYPE(VertexPair(vfi.V(), vfi.V1()), MarkUpDating(), pp, qh)));
						std::push_heap(h_ret.begin(), h_ret.end());
						if (!this->IsSymmetric(pp)) {
							h_ret.push_back(HeapElem(new MYTYPE(VertexPair(vfi.V1(), vfi.V()), MarkUpDating(), pp, qh)));
							std::push_heap(h_ret.begin(), h_ret.end());
						}
					}
					if (!(vfi.V2()->IsV()) && (vfi.V2()->IsRW()))
					{
						vfi.V2()->SetV();
						h_ret.push_back(HeapElem(new MYTYPE(VertexPair(vfi.F()->V(vfi.I()), vfi.F()->V2(vfi.I())), MarkUpDating(), pp, qh)));
						std::push_heap(h_ret.begin(), h_ret.end());
						if (!this->IsSymmetric(pp)) {
							h_ret.push_back(HeapElem(new MYTYPE(VertexPair(vfi.F()->V1(vfi.I()), vfi.F()->V(vfi.I())), MarkUpDating(), pp, qh)));
							std::push_heap(h_ret.begin(), h_ret.end());
						}
					}
					//        if(vfi.V1()->IsRW() && vfi.V2()->IsRW() )
			...
			template <class LocalTexHelperType>
			static void Init(TriMeshType& m, HeapType& h_ret, BaseParameterClass* pp, LocalTexHelperType& qh)
			{
				vcg::tri::RequirePerVertexMark(m);
				vcg::tri::UpdateTopology<TriMeshType>::VertexFace(m);
				h_ret.clear();
				...
				for (fi = m.face.begin(); fi != m.face.end(); ++fi)
				{
					if (!(*fi).IsD()) {
						for (int j = 0; j < 3; j++)
						{
							VertexPair p((*fi).V0(j), (*fi).V1(j));
							p.Sort();
							//h_ret.push_back(HeapElem(new MYTYPE(p, IMark(m), pp, qh)));
							//printf("Inserting in heap coll %3i ->%3i %f\n",p.V()-&m.vert[0],p.VFlip()-&m.vert[0],h_ret.back().locModPtr->Priority());
						}
					}
				}
